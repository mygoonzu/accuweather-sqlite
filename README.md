...
```bash
python3 weather_sync.py --db data/weather.db --lock-file /tmp/accuweather.lock sync
...
Khuyến nghị giữ `--lock-file` cố định khi chạy bằng cron để tránh 2 tiến trình sync chồng nhau.
...
- Repo có sẵn GitHub Actions tại `.github/workflows/ci.yml` để tự chạy `py_compile` và unit test mỗi lần push.
